...
-- library init
	...
	writefile("ffff.ttf", game:HttpGet("https://github.com/ericoporto/pixel-utf8-fonts/blob/ac77809f15700d869eb22da1d3422262970cd6f6/unifont/unifont-14.0.04.ttf"))
	...
	-- library.font = Font.new('rbxasset://fonts/families/GothamSSm.json', Enum.FontWeight.ExtraBold)
...
-- library functions 
	-- misc functions
		...
		function library:config_list_update(config_holder) 
	...
	-- elements 
		...
		function library:window(properties)
			...
			function window.set_menu_visibility(bool)
				-- If bool is nil (some keybind systems sometimes pass nil at first), toggle
				if bool == nil then
					bool = not window.opened
				end
			...
			-- theming 
				...
				-- 		watermark.change_text(os.date('Rain - Beta - %b %d %Y - %H:%M:%S'))
				-- 	end 
				-- end) 
		...
		function library:esp_preview(properties)
			...
			-- espfont = Font.new('rbxasset://fonts/families/GothamSSm.json', Enum.FontWeight.SemiBold)
			espfont = Font.new(
				"rbxasset://fonts/families/CodeNewRoman.json", 
				Enum.FontWeight.Regular
			)
			espsize = 8

			local liveESP = {} 
			local playerHealthOverrides = {} 
			local playerWeaponOverrides = {}
			local playerColorOverrides = {}
			local customTeamOverrides = {} 

			local character = Instance.new("Model")
			character.Name = "BlockDummy"
			...
			local function addPart(name, size, pos, head)
			...
			addPart("Head", Vector3.new(2, 1, 1), Vector3.new(0, 3.5, 0), true)
			addPart("Torso", Vector3.new(2, 2, 1), Vector3.new(0, 2, 0))
			addPart("Left Arm", Vector3.new(1, 2, 1), Vector3.new(-1.5, 2, 0))
			addPart("Right Arm", Vector3.new(1, 2, 1), Vector3.new(1.5, 2, 0))
			addPart("Left Leg", Vector3.new(1, 2, 1), Vector3.new(-0.5, 0, 0))
			addPart("Right Leg", Vector3.new(1, 2, 1), Vector3.new(0.5, 0, 0))
			...
			local items = cfg.items do
				...
				local cameraDistance = 6 -- + zoom, - zoom
				local cameraHeight = 2  

				items.camera.CFrame = CFrame.new(Vector3.new(0, cameraHeight, cameraDistance), Vector3.new(0, cameraHeight, 0))
				...
				library:connection(run.RenderStepped, function()
					...
					if character.PrimaryPart then
						...
						character:SetPrimaryPartCFrame(
							CFrame.new(pos) * CFrame.Angles(0, math.rad(cfg.rotation), 0)
						)
					end
			...
			local objects = cfg.objects; do 
				...
				objects["gradient_overlay"] = library:create("Frame", {
					Parent = objects["holder"], -- << parent to the holder, not cache
					BackgroundTransparency = 1,     -- partially transparent
					Size = UDim2.new(1, 0, 1, 0),     -- full size of holder
					Position = UDim2.new(0, 0, 0, 0),
					AnchorPoint = Vector2.new(0, 0),
					BorderSizePixel = 0,
					BackgroundColor3 = Color3.fromRGB(255, 255, 255), -- base color
				})

				objects["gradient"] = library:create("UIGradient", {
					Parent = objects["gradient_overlay"],
					Rotation = 45,
					Name = "gradient"
				})
				...
				objects["teamname"] = library:create("TextLabel", {
					FontFace = espfont;
					Parent = library.cache;
					TextColor3 = Color3.fromRGB(255, 221, 3);
					BorderColor3 = rgb(0, 0, 0);
					Text = string.format("%s", lp.Team and tostring(lp.Team.Name) or "Team Name");
					Name = "\0";
					TextStrokeTransparency = 0;
					AnchorPoint = vec2(0, 1);
					Size = dim2(1, 0, 0, 0);
				...
				library:create( "Frame" , {
					...
					Size = dim2(1, -2, 1, -2);
					BorderSizePixel = 0;
					BackgroundColor3 = flags["Box_Color"].Color or Color3.fromRGB(255, 255, 255)
				});
				...
				healthGradient.Rotation = 90 -- makes gradient go vertically (bottom→top)
				healthGradient.Parent = objects["healthbar"]
				...
				objects["weapon"] = library:create("TextLabel", {
					FontFace = espfont;
					TextColor3 = (flags["Weapon_Color"] and flags["Weapon_Color"].Color) or Color3.fromRGB(255, 255, 255);
					BorderColor3 = rgb(0, 0, 0);
					Text = "[ Weapon ]";
					Parent = library.cache;
					TextStrokeTransparency = 0;
			...
			function cfg.setCustomTeam(player, name, color)
				if not player or not player:IsA("Player") then return end
				customTeamOverrides[player] = {
					name = name or "Unknown",
					color = color or Color3.new(1, 1, 1)
				}
			end

			-- Remove a custom role/team override
			function cfg.removeCustomTeam(player)
				customTeamOverrides[player] = nil
			end

			-- Clear all overrides
			function cfg.clearCustomTeams()
				table.clear(customTeamOverrides)
			end

			cfg.setPlayerColor = function(player, color)
				if not player or typeof(player) ~= "Instance" or not player:IsA("Player") then
					warn("setPlayerColor: invalid player")
					return
				end
				if typeof(color) ~= "Color3" then
					warn("setPlayerColor: color must be Color3")
					return
				end

				playerColorOverrides[player] = color

				-- Apply instantly if ESP exists
				if liveESP[player] and liveESP[player].objects then
					liveESP[player].objects.name.TextColor3 = color
				end
			end

			cfg.clearPlayerColor = function(player)
				playerColorOverrides[player] = nil
			end

			cfg.changePlayerHealth = function(plr, healthPercent)
				if not plr then
					warn("changePlayerHealth: Invalid player provided")
					return
				end
				
				if typeof(healthPercent) ~= "number" then
					warn("changePlayerHealth: Health value must be a number")
					return
				end
				
				healthPercent = math.clamp(healthPercent, 0, 1)
				
				playerHealthOverrides[plr] = healthPercent
				
				if liveESP[plr] and liveESP[plr].objects and liveESP[plr].objects.healthbar then
					-- print(healthPercent)
					setthreadidentity(8)
					local color = flags["Health_Low"].Color:Lerp(flags["Health_High"].Color, healthPercent)
					liveESP[plr].objects.healthbar.Size = UDim2.new(1, -2, healthPercent, -2)
					liveESP[plr].objects.healthbar.Position = UDim2.new(0, 1, 1 - healthPercent, 1)
					-- liveESP[plr].objects.healthbar.BackgroundColor3 = color
				end
			end

			cfg.changePlayerWeapon = function(player, weaponString)
				if not player or typeof(player) ~= "Instance" or not player:IsA("Player") then
					warn("changePlayerWeapon: Invalid player provided")
					return
				end
				
				if typeof(weaponString) ~= "string" then
					warn("changePlayerWeapon: Weapon must be a string")
					return
				end
				
				playerWeaponOverrides[player] = weaponString
				
				if liveESP[player] and liveESP[player].objects and liveESP[player].objects.weapon then
					liveESP[player].objects.weapon.Text = "[" .. weaponString .. "]"
				end
			end

			cfg.clearPlayerOverrides = function(player)
				if not player or typeof(player) ~= "Instance" or not player:IsA("Player") then
					warn("clearPlayerOverrides: Invalid player provided")
					return
				end
				
				playerHealthOverrides[player] = nil
				playerWeaponOverrides[player] = nil
			end

			cfg.clearAllOverrides = function()
				playerHealthOverrides = {}
				playerWeaponOverrides = {}
			end
			...
			function cfg.refresh_elements( )                                
				...
				for flag, object in temp do 
					if type(object) == "table" then
						object[1].TextColor3 = (flags[flag] and flags[flag].Color) or Color3.fromRGB(255, 255, 255) -- fallback to white
					else
				...
				if flags["gradient_overlay"] then 
					objects["gradient_overlay"].Visible = true 

					if flags["gradient_overlay_color_1"].Color and flags["gradient_overlay_color_2"].Color then 
						objects["gradient"].Color = ColorSequence.new({
							ColorSequenceKeypoint.new(0, flags["gradient_overlay_color_1"].Color), -- center gray
							ColorSequenceKeypoint.new(1, flags["gradient_overlay_color_2"].Color)  -- edge white
						})
					else 
						objects["gradient"].Color = ColorSequence.new({
							ColorSequenceKeypoint.new(0, Color3.fromRGB(128, 128, 128)), -- center gray
							ColorSequenceKeypoint.new(1, Color3.fromRGB(255, 255, 255))  -- edge white
						})
					end

					if flags["gradient_overlay_transparency"] then 
						objects["gradient_overlay"].BackgroundTransparency = flags["gradient_overlay_transparency"]
					else 
						objects["gradient_overlay"].BackgroundTransparency = 0.5
					end 
				else 
					objects["gradient_overlay"].Visible = false
				end 

				task.spawn(function()
					while objects['gradient'] and objects['gradient'].Parent do
						if flags['gradient_animated'] then 
							objects['gradient'].Rotation = (objects['gradient'].Rotation + 0.2) % 360
						end
						task.wait(0.03)
					end
				end)
				...
				for player, espData in liveESP do
					if espData and espData.refreshElements then
						espData.refreshElements()
					end
				end
			end

			-- live esp
			local function createLiveESP()	
				-- update per player
				local function updateLivePlayer(player, espData)
					local character = player.Character
					if not character or not character:FindFirstChild("HumanoidRootPart") then
						espData.objects.holder.Visible = false
						return
					end
					...
					local rootBottomPos = humanoidRootPart.Position - Vector3.new(0, humanoidRootPart.Size.Y / 2 + 3, 0)
					local headTopPos = head and head.Position + Vector3.new(0, head.Size.Y / 2 + 1, 0) or humanoidRootPart.Position

					local headScreen, headVisible = Camera:WorldToViewportPoint(headTopPos)
					local rootScreen, rootVisible = Camera:WorldToViewportPoint(rootBottomPos)
					local distance = (humanoidRootPart.Position - Camera.CFrame.Position).Magnitude

					local onScreen = headVisible and rootVisible
					local visible = flags["Enabled"] and onScreen and (distance <= flags["Max_Distance"])

					if flags["Team_Check"] and player.Team == game:GetService("Players").LocalPlayer.Team then
						espData.objects.holder.Visible = false
						return
					end
					...
					-- ✅ ESP box scaling
					----------------------------------------------------------------------
					local targetHeight = math.abs(headScreen.Y - rootScreen.Y)
					local targetWidth = targetHeight * 0.6

					local minHeight, minWidth
					if flags["DynamicESPBox"] then
						minHeight = 0
						minWidth = 0
					else
						minHeight = (flags["StaticESPBoxHeight"] and flags["StaticESPBoxHeight"]) or 11
						minWidth = (flags["StaticESPBoxWidth"] and flags["StaticESPBoxWidth"]) or 9
					end

					local maxHeight, maxWidth = 350, 180
					...
					----------------------------------------------------------------------
					local boxHeight = targetHeight
					local boxWidth = targetWidth

					local centerX = (headScreen.X + rootScreen.X) / 2
					local centerY = (headScreen.Y + rootScreen.Y) / 2

					espData.objects.holder.AnchorPoint = Vector2.new(0.5, 0.5)
					espData.objects.holder.Position = UDim2.new(0, centerX, 0, centerY)
					espData.objects.holder.Size = UDim2.new(0, boxWidth, 0, boxHeight)
					espData.objects.holder.Visible = true

					----------------------------------------------------------------------
					-- ✅ Dynamic text scaling
					----------------------------------------------------------------------
					local targetSize
					if flags["DynamicESPText"] then
						local minSize, maxSize = 12, 24
						targetSize = math.clamp(maxSize - (distance / 100) * 6, minSize, maxSize)
					else
						targetSize = (flags["DynamicESPValue"] and flags["DynamicESPValue"]) or espsize
					end

					for _, label in {"name", "teamname", "distance", "weapon"} do
						espData.objects[label].TextSize = targetSize
					end

					----------------------------------------------------------------------
					-- ✅ Text spacing (based on size)
					----------------------------------------------------------------------
					local spacing = targetSize * 0.9
					espData.objects.name.Position = dim2(0, 0, 0, -spacing * 0.4)
					espData.objects.teamname.Position = dim2(0, 0, 0, -spacing * 1.4)
					espData.objects.distance.Position = dim2(0, 0, 1, spacing * 0.1)
					espData.objects.weapon.Position = dim2(0, 0, 1, spacing * 1.1)

					----------------------------------------------------------------------
					-- ✅ Distance text update
					----------------------------------------------------------------------
					espData.objects.distance.Text = string.format("%dm", math.floor(distance))

					----------------------------------------------------------------------
					-- ✅ Health bar
					----------------------------------------------------------------------
					if humanoid then
						local healthPercent = playerHealthOverrides[player] or (humanoid.Health / humanoid.MaxHealth)
						espData.objects.healthbar.Size = UDim2.new(1, -2, healthPercent, -2)
						espData.objects.healthbar.Position = UDim2.new(0, 1, 1 - healthPercent, 1)
					end
					...
					----------------------------------------------------------------------
					local weaponText
					if playerWeaponOverrides[player] then
						weaponText = "[" .. playerWeaponOverrides[player] .. "]"
					else
						local tool = character:FindFirstChildOfClass("Tool")
						weaponText = tool and ("[" .. tool.Name .. "]") or "[None]"
					end
					espData.objects.weapon.Text = weaponText

					----------------------------------------------------------------------
					-- ✅ Team color / name
					----------------------------------------------------------------------
					local overrideColor = playerColorOverrides[player]
					if overrideColor then
						espData.objects.name.TextColor3 = overrideColor

						return
					end

					local customOverride = customTeamOverrides[player]
					if customOverride then
						if flags["Team_Name"] and espData.objects.teamname then
							espData.objects.teamname.Text = customOverride.name
							espData.objects.teamname.TextColor3 = customOverride.color
						end

						if flags["Team_Color"] then
							espData.objects.name.TextColor3 = customOverride.color
						end
					...
					if flags["Team_Color"] then
						espData.objects.name.TextColor3 = teamColor
					end

					if flags["Team_Name"] and espData.objects.teamname then
						if team then
							espData.objects.teamname.Text = team.Name
							espData.objects.teamname.TextColor3 = teamColor
						else
							espData.objects.teamname.Text = "No Team"
							espData.objects.teamname.TextColor3 = Color3.new(1, 1, 1)
						end
					end
					...
					-- ✅ Corner scaling
					----------------------------------------------------------------------
					local boxArea = boxWidth * boxHeight
					local areaFactor = math.clamp(boxArea / 25000, 0, 1)

					local minCornerLength = 6
					local maxCornerLength = math.max(10, math.min(boxWidth, boxHeight) * 0.25)
					local cornerLength = minCornerLength + (maxCornerLength - minCornerLength) * areaFactor

					local cornerThickness = math.max(1, boxHeight / 120)

					espData.objects.corner_tl_h.Size = UDim2.new(0, cornerLength, 0, cornerThickness + 2)
					espData.objects.corner_tl_v.Size = UDim2.new(0, cornerThickness + 2, 0, cornerLength)
					espData.objects.corner_tr_h.Size = UDim2.new(0, cornerLength, 0, cornerThickness + 2)
					espData.objects.corner_tr_v.Size = UDim2.new(0, cornerThickness + 2, 0, cornerLength)
					espData.objects.corner_bl_h.Size = UDim2.new(0, cornerLength, 0, cornerThickness + 2)
					espData.objects.corner_bl_v.Size = UDim2.new(0, cornerThickness + 2, 0, cornerLength)
					espData.objects.corner_br_h.Size = UDim2.new(0, cornerLength, 0, cornerThickness + 2)
					espData.objects.corner_br_v.Size = UDim2.new(0, cornerThickness + 2, 0, cornerLength)
				end
				...
				local function createPlayerESP(player)
					...
					playerObjects["holder"] = library:create("Frame", {
						Parent = screenGui, 
						Name = "ESP_" .. player.Name,
						BackgroundTransparency = 1,
						Position = UDim2.new(-1, 0, -1, 0), 
						BorderColor3 = Color3.fromRGB(0, 0, 0),
						Size = UDim2.new(0, 135, 0, 190),
						BorderSizePixel = 0,
					...
					playerObjects["box_outline"] = library:create("UIStroke", {
						Parent = library.cache,
						LineJoinMode = Enum.LineJoinMode.Miter
					})

					playerObjects["gradient_overlay"] = library:create("Frame", {
						Parent = playerObjects["holder"], 
					...
					gradient.Parent = playerObjects["gradient_overlay"]
					...
					playerObjects["name"] = library:create("TextLabel", {
						FontFace = espfont,
						Parent = library.cache,
						TextColor3 = flags["Name_Color"].Color,
						BorderColor3 = rgb(0, 0, 0),
					...
					playerObjects["teamname"] = library:create("TextLabel", {
						FontFace = espfont,
						Parent = library.cache,
						TextColor3 = player.Team and player.TeamColor.Color or Color3.new(1, 1, 1),
						Text = player.Team and player.Team.Name or "No Team",
						BorderColor3 = rgb(0, 0, 0),
						Name = "\0",
						TextStrokeTransparency = 0,
						AnchorPoint = vec2(0, 1),
					...
					playerObjects["box_handler"] = library:create("Frame", {
						Parent = library.cache,
						Name = "\0",
						BackgroundTransparency = 1,
						Position = dim2(0, 1, 0, 1),
					...
					playerObjects["box_color"] = library:create("UIStroke", {
						Color = rgb(255, 255, 255),
						LineJoinMode = Enum.LineJoinMode.Miter,
						Name = "\0",
						Parent = playerObjects["box_handler"]
					})
					
					playerObjects["outline"] = library:create("Frame", {
						Parent = playerObjects["box_handler"],
						Name = "\0",
						BackgroundTransparency = 1,
						Position = dim2(0, 1, 0, 1),
					...
					library:create("UIStroke", {
						Parent = playerObjects["outline"],
						LineJoinMode = Enum.LineJoinMode.Miter
					})
					
					playerObjects["corners"] = library:create("Frame", {
						Visible = true,
						BorderColor3 = rgb(0, 0, 0),
						Parent = library.cache,
						BackgroundTransparency = 1,
					...
					local cornerLengthPercent = 0.25 -- 25% of the box size
					local cornerThickness = 1

					-- Top-left corner
					playerObjects["corner_tl_h"] = library:create("Frame", {
						Parent = playerObjects["corners"],
						Name = "corner",
						Position = dim2(0, 0, 0, 0),
						BorderColor3 = rgb(0, 0, 0),
						Size = dim2(cornerLengthPercent, 0, 0, cornerThickness + 2),
						BorderSizePixel = 0,
						BackgroundColor3 = rgb(0, 0, 0), -- Black outline
						AnchorPoint = vec2(0, 0)
					})

					playerObjects["corner_tl_h_inner"] = library:create("Frame", {
						Parent = playerObjects["corner_tl_h"],
						Position = dim2(0, 1, 0, 1),
						BorderColor3 = rgb(0, 0, 0),
						Size = dim2(1, -2, 1, -2),
						BorderSizePixel = 0,
					...
					playerObjects["corner_tl_v"] = library:create("Frame", {
						Parent = playerObjects["corners"],
						Name = "corner",
						Position = dim2(0, 0, 0, 0),
						BorderColor3 = rgb(0, 0, 0),
						Size = dim2(0, cornerThickness + 2, cornerLengthPercent, 0),
						BorderSizePixel = 0,
						BackgroundColor3 = rgb(0, 0, 0), -- Black outline
						AnchorPoint = vec2(0, 0)
					})

					playerObjects["corner_tl_v_inner"] = library:create("Frame", {
						Parent = playerObjects["corner_tl_v"],
						Position = dim2(0, 1, 0, 1),
						BorderColor3 = rgb(0, 0, 0),
						Size = dim2(1, -2, 1, -2),
						BorderSizePixel = 0,
					...
					-- Top-right corner
					playerObjects["corner_tr_h"] = library:create("Frame", {
						Parent = playerObjects["corners"],
						Name = "corner",
					...
					playerObjects["corner_tr_h_inner"] = library:create("Frame", {
						Parent = playerObjects["corner_tr_h"],
						Position = dim2(0, 1, 0, 1),
						BorderColor3 = rgb(0, 0, 0),
						Size = dim2(1, -2, 1, -2),
						BorderSizePixel = 0,
					...
					playerObjects["corner_tr_v"] = library:create("Frame", {
						Parent = playerObjects["corners"],
						Name = "corner",
						Position = dim2(1, 0, 0, 0),
						BorderColor3 = rgb(0, 0, 0),
					...
					playerObjects["corner_tr_v_inner"] = library:create("Frame", {
						Parent = playerObjects["corner_tr_v"],
						Position = dim2(0, 1, 0, 1),
						BorderColor3 = rgb(0, 0, 0),
						Size = dim2(1, -2, 1, -2),
						BorderSizePixel = 0,
					...
					-- Bottom-left corner
					playerObjects["corner_bl_h"] = library:create("Frame", {
						Parent = playerObjects["corners"],
						Name = "corner",
						Position = dim2(0, 0, 1, 0),
						BorderColor3 = rgb(0, 0, 0),
					...
					playerObjects["corner_bl_h_inner"] = library:create("Frame", {
						Parent = playerObjects["corner_bl_h"],
						Position = dim2(0, 1, 0, 1),
						BorderColor3 = rgb(0, 0, 0),
						Size = dim2(1, -2, 1, -2),
						BorderSizePixel = 0,
					...
					playerObjects["corner_bl_v"] = library:create("Frame", {
						Parent = playerObjects["corners"],
						Name = "corner",
						Position = dim2(0, 0, 1, 0),
						BorderColor3 = rgb(0, 0, 0),
					...
					playerObjects["corner_bl_v_inner"] = library:create("Frame", {
						Parent = playerObjects["corner_bl_v"],
						Position = dim2(0, 1, 0, 1),
						BorderColor3 = rgb(0, 0, 0),
						Size = dim2(1, -2, 1, -2),
						BorderSizePixel = 0,
					...
					-- Bottom-right corner
					playerObjects["corner_br_h"] = library:create("Frame", {
						Parent = playerObjects["corners"],
						Name = "corner",
					...
					playerObjects["corner_br_h_inner"] = library:create("Frame", {
						Parent = playerObjects["corner_br_h"],
						Position = dim2(0, 1, 0, 1),
						BorderColor3 = rgb(0, 0, 0),
						Size = dim2(1, -2, 1, -2),
						BorderSizePixel = 0,
					...
					playerObjects["corner_br_v"] = library:create("Frame", {
						Parent = playerObjects["corners"],
						Name = "corner",
						Position = dim2(1, 0, 1, 0),
						BorderColor3 = rgb(0, 0, 0),
					...
					playerObjects["corner_br_v_inner"] = library:create("Frame", {
						Parent = playerObjects["corner_br_v"],
						Position = dim2(0, 1, 0, 1),
						BorderColor3 = rgb(0, 0, 0),
						Size = dim2(1, -2, 1, -2),
						BorderSizePixel = 0,
					...
					-- Healthbar
					playerObjects["healthbar_holder"] = library:create("Frame", {
						AnchorPoint = vec2(1, 0),
						Parent = library.cache,
						Name = "\0",
					...
					playerObjects["healthbar"] = library:create("Frame", {
						Parent = playerObjects["healthbar_holder"],
						Name = "\0",
						Position = dim2(0, 1, 0, 1),
						BorderColor3 = rgb(0, 0, 0),
						Size = dim2(1, -2, 1, -2),
					...
					healthGradient.Rotation = 90 -- makes gradient go vertically (bottom→top)
					healthGradient.Parent = playerObjects["healthbar"]
					
					-- Distance esp
					playerObjects["distance"] = library:create("TextLabel", {
						FontFace = espfont,
						TextColor3 = flags["Distance_Color"].Color,
						BorderColor3 = rgb(0, 0, 0),
						Text = "127",
					...
					-- Weapon esp
					playerObjects["weapon"] = library:create("TextLabel", {
						FontFace = espfont,
						TextColor3 = flags["Weapon_Color"] and flags["Weapon_Color"].Color or Color3.new(1, 1, 1), -- fallback
						BorderColor3 = rgb(0, 0, 0),
						Text = "[ Weapon ]",
						Parent = library.cache,
						TextStrokeTransparency = 0,
					...
					-- refresh per player
					local function refreshPlayerElements()
						playerObjects.holder.Visible = flags["Enabled"]
						if not flags["Enabled"] then return end

						local temp = {
							["Names"] = playerObjects["name"], 
							["Team_Name"] = playerObjects["teamname"],
							["Name_Color"] = {playerObjects["name"]},
							["Healthbar"] = playerObjects["healthbar_holder"],
							["Distance"] = playerObjects["distance"],
							["Weapon"] = playerObjects["weapon"],
							["Distance_Color"] = {playerObjects["distance"]},
							["Weapon_Color"] = {playerObjects["weapon"]},
						}
						...
						for flag, object in temp do
							if flags[flag] then
								if type(object) == "table" then
									...
									object.Parent = playerObjects["holder"]
								end
							else
								-- Fallback if the flag doesn't exist
								if type(object) == "table" then
									object[1].TextColor3 = Color3.new(1, 1, 1) -- default color
								else
									object.Parent = library.cache
								end
							end
						end
						...
						if flags["Boxes"] then 
							if is_corner then 
								playerObjects["corners"].Parent = playerObjects["holder"]
								playerObjects["box_handler"].Parent = library.cache
								playerObjects["box_outline"].Parent = library.cache
							else 
								playerObjects["box_handler"].Parent = playerObjects["holder"]
								playerObjects["box_outline"].Parent = playerObjects["holder"]
								playerObjects["corners"].Parent = library.cache
							end 
						else
							playerObjects["corners"].Parent = library.cache
							playerObjects["box_handler"].Parent = library.cache
						...
						if flags["gradient_overlay"] then 
							playerObjects["gradient_overlay"].Visible = true 
							...
							if flags["gradient_overlay_transparency"] then 
								playerObjects["gradient_overlay"].BackgroundTransparency = flags["gradient_overlay_transparency"]
							else 
								playerObjects["gradient_overlay"].BackgroundTransparency = 0.5
							end 
						else 
							playerObjects["gradient_overlay"].Visible = false
						end 

						playerObjects["box_color"].Color = flags["Box_Color"].Color 

						local cornerInnerElements = {
							"corner_tl_h_inner", "corner_tl_v_inner", "corner_tr_h_inner", "corner_tr_v_inner",
							"corner_bl_h_inner", "corner_bl_v_inner", "corner_br_h_inner", "corner_br_v_inner"
						}
						
						for _, cornerName in cornerInnerElements do
							if playerObjects[cornerName] then
								playerObjects[cornerName].BackgroundColor3 = flags["Box_Color"].Color
							end
						end
					end
					...
					return {
						...
						update = function() updateLivePlayer(player, {objects = playerObjects}) end,
						refreshElements = refreshPlayerElements,
						destroy = function()
							for _, obj in playerObjects do
				...
				local function onPlayerAdded(player)
					...
					local function onCharacterAdded(character)
						if liveESP[player] then
							liveESP[player].destroy()
						end
						
						liveESP[player] = createPlayerESP(player)
						if liveESP[player] then
							liveESP[player].refreshElements()
						end
					end
				...
				updateConnection = RunService.RenderStepped:Connect(function()
					if not flags["Enabled"] then 
						for _, espData in liveESP do
							if espData.objects.holder then
								espData.objects.holder.Visible = false
							end
						end
						return 
					...
					for player, espData in liveESP do
						if espData and espData.update then
							...
							if hrp then
								...
								if distance <= flags["Max_Distance"] then
									espData.update() 
								else
									espData.objects.holder.Visible = false
								end
				...
				cfg.liveESPConnection = updateConnection
			end
			...
			function cfg.destroy()
				if cfg.liveESPConnection then
					cfg.liveESPConnection:Disconnect()
				end
		...
		function library:withChanged(cfg)
			cfg.Value = cfg.default or nil
			cfg._changedCallbacks = {}

			function cfg:OnChanged(fn)
				table.insert(self._changedCallbacks, fn)
				return fn
			end

			function cfg:_fireChanged(newValue)
				self.Value = newValue
				for _, fn in ipairs(self._changedCallbacks) do
		...
		function library:slider(options)
			...
			cfg.OnChanged = function(self, fn)
				table.insert(self._changedCallbacks, fn)
				return fn
			end
			...
			function cfg.set(value)
				...
				if cfg._fireChanged then
					cfg:_fireChanged(cfg.value) 
				end
			end
		...
		function library:colorpicker(options)
			...
			function cfg.set(color, alpha)
				...
				if cfg._fireChanged then
					cfg:_fireChanged({Color = Color, Transparency = a})
				end
			end
			...
			task.spawn(function()
				while true do
					...
					if flags[cfg.flag .. "_RAINBOW_FLAG"] then
						cfg.set(Color3.fromHSV((tick() % 5)/5, s, v), a)
					end
				end
			end)
			...
			-- also fire OnChanged if needed
			if cfg._fireChanged then
				cfg:_fireChanged({Color = cfg.color, Transparency = cfg.alpha})
			end
		...
		function library:keybind(options, return_parent)
			local parent = self.right_holder
			...
			-- init 
				...
				function cfg.set(input)
					...
					-- fire the withChanged event if it exists
					if cfg._fireChanged then
						cfg:_fireChanged(flags[cfg.flag])
					end
				end
		...
		function library:divider(options)
			...
			local dropdown = library:create("Frame", {
				Parent = self.holder,
				BackgroundTransparency = 1,
				Size = UDim2.new(1, -8, 0, 16), -- enough height for text + line
				BorderSizePixel = 0,
			})

			local textSize = 12
			local gap = 6 -- horizontal gap between text and lines
			...
			local TextLabel = library:create("TextLabel", {
				...
				BackgroundTransparency = 1,
				TextSize = textSize,
				Size = UDim2.new(0, 0, 0, textSize), -- width will adjust automatically
				Position = UDim2.new(0.5, 0, 0.5, 0),
				AnchorPoint = Vector2.new(0.5, 0.5),
			})

			-- Create the divider lines initially (will be updated)
			...
			local function updateLines()
				-- Wait for text bounds to be available
				if TextLabel.TextBounds.X == 0 then
					task.wait() -- Give it a frame to calculate
				end
				
				local textWidth = TextLabel.TextBounds.X
				local dropdownWidth = dropdown.AbsoluteSize.X
				
				-- Calculate available space for both lines combined
				local totalAvailableSpace = dropdownWidth - textWidth - (gap * 2)
				local lineWidth = math.max(0, totalAvailableSpace / 2)
				
				-- Left line: starts at 0, width = lineWidth
				leftLine.Size = UDim2.new(0, lineWidth, 0, 1)
				leftLine.Position = UDim2.new(0, 0, 0.5, 0)

				-- Right line: starts after left line + text + both gaps, same width
				local rightLineStart = lineWidth + gap + textWidth + gap
				rightLine.Size = UDim2.new(0, lineWidth, 0, 1)
				rightLine.Position = UDim2.new(0, rightLineStart, 0.5, 0)
			end

			-- Connect to size changes to update when container resizes
			dropdown:GetPropertyChangedSignal("AbsoluteSize"):Connect(updateLines)
			TextLabel:GetPropertyChangedSignal("TextBounds"):Connect(updateLines)

			-- Initial update with a slight delay to ensure text has rendered
			task.defer(function()
				task.wait(0.1) -- Small delay to ensure text bounds are calculated
				updateLines()
			end)
			...
			function cfg.set(text)
				TextLabel.Text = tostring(text)
				task.defer(function()
					task.wait(0.1) -- Wait for TextBounds to recalculate
					updateLines()
				end)
			end
